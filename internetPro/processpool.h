...
class processpool
{
	private:
		...
		processpool(int lis,int max_son=8);
	public:
		static processpool<T>* create(int lis,int max_son=8)
		{
			if(!m_processpool)
			{
				m_processpool=new processpool<T>(lis,max_son);
			}
...
template<typename T>
processpool<T>::processpool(int lis,int max_son):
